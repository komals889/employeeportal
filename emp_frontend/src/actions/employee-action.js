...
export const allUserAction=()=>async(dispatch,getState)=>{
    try {
        ...
        const {data}=await axios.get("http://localhost:5000/get-employee",config)
...
export const updateUserAction = (id,formData)=>async(dispatch,getState)=>{
    try {
       ...
       const {data}=await axios.put(`http://localhost:5000/update-employee/${id}`,formData,config);
       console.warn(data)
...
export const deleteUserAction = (id)=>async(dispatch,getState)=>{
    try {
       ...
       const {data}=await axios.delete(`http://localhost:5000/delete-employee/${id}`,config);
